(function () {
    ...
    /* Oracle NLS brasileiro pode retornar "26,08" com vírgula como decimal.
       Converte para ponto antes de parseFloat. */
    function toNum(v) {
        if (v === null || v === undefined) return NaN;
        if (typeof v === 'number') return v;
        // "1.234,56" → remove pontos de milhar → "1234,56" → troca vírgula por ponto
        const s = String(v).replace(/\.(?=\d{3})/g, '').replace(',', '.');
        return parseFloat(s);
    }

    function fmtNum(v) { const n = toNum(v); return isNaN(n) ? '' : (n === 0 ? '0' : n); }
    function fmtDec2(v) {
        const n = toNum(v);
    ...
    /* ── ROLO BORDADO (detalhe — tabela única com subtotal por ORD) ── */
    function renderRoloBordado(rows, numLote, dataLote) {
        ...
        function flushOrd() {
            ...
            <tr class="subtotal-row">
                <td colspan="4" style="text-align:right;font-weight:bold">SUBTOTAL ${ordAtual}:</td>
                <td style="text-align:right;font-weight:bold">${fmtDec2(subLinear)}</td>
                <td style="text-align:right;font-weight:bold">${fmtDec4(subFatias)}</td>
            </tr>`;
            buf.length = 0;
            subLinear  = 0;
            subFatias  = 0;
        ...
        for (const r of rows) {
            const ord = fmt(r.ORDEM);
            if (ord !== ordAtual) {
                flushOrd();
                ordAtual = ord;
            }
            subLinear += toNum(r.MT_LINEAR) || 0;
            subFatias += toNum(r.FATIAS)    || 0;
            buf.push(`
            <tr>
                <td class="td-wrap">${fmt(r.SEMIFAIXA)}</td>
...
<div class="pcp-section">
    ...
    /* ── ROLO BORDADO AGRUPADO (existente) ─────────────────── */
    function renderRoloBordadoAgrupado(rows, numLote, dataLote) {
        if (!rows || rows.length === 0) return '';
...
<meta charset="utf-8">
<title>Sequência de Produção — Rolo Bordado</title>
<style>
  ...
  .pcp-section:last-child{page-break-after:avoid}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-rolo-bordado', {
                    method:  'POST',
                ...
                let html  = renderRoloBordado(data.detalhe_rows || [], numLote, dt);
                html     += renderRoloBordadoAgrupado(data.rolo_rows || [], numLote, dt);
