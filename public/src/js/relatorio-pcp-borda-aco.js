...
<div class="pm-header-wrap">
    <div class="pm-header-row1">
        ...
        <div class="col-title">RELATÓRIO DE PRODUÇÃO — BORDA DE AÇO — PCP MOLAS</div>
...
<tr>
    ...
    <td>${fmt(r.COD_ITEM_BORDA)}</td>
    <td class="td-wrap">${fmt(r.DESC_BORDA)}</td>
    <td class="td-wrap">${fmt(r.MASCARA_BORDA)}</td>
    <td style="text-align:center;font-weight:bold">${fmtN(r.TOTAL_BORDA)}</td>
</tr>`;
        }

        // ── SEÇÃO 2: AGRUPADO POR BORDA ─────────────────────────────
        const mapaGrupos = new Map();
        for (const r of rows) {
            const chave = `${r.COD_ITEM_BORDA}||${r.DESC_BORDA}||${r.MASCARA_BORDA}`;
            if (!mapaGrupos.has(chave)) {
                mapaGrupos.set(chave, {
                    COD_ITEM_BORDA: r.COD_ITEM_BORDA,
                    DESC_BORDA:     r.DESC_BORDA,
                    MASCARA_BORDA:  r.MASCARA_BORDA,
                    TOTAL:          0,
                });
            }
            mapaGrupos.get(chave).TOTAL += parseFloat(r.TOTAL_BORDA) || 0;
        }
        ...
        for (const g of mapaGrupos.values()) {
            ...
            corpoAgrup += `
<tr>
    <td>${fmt(g.COD_ITEM_BORDA)}</td>
    <td class="td-wrap">${fmt(g.DESC_BORDA)}</td>
    <td class="td-wrap">${fmt(g.MASCARA_BORDA)}</td>
    <td style="text-align:center;font-weight:bold">${fmtN(g.TOTAL)}</td>
</tr>`;
        }
        corpoAgrup += `
<tr class="total-row">
    <td colspan="3" style="text-align:right;padding-right:8px">TOTAL GERAL DE BORDAS:</td>
...
<div class="pm-section">
    ...
    <div class="pm-section-title" style="margin-top:14px">${titulo} — AGRUPADO POR BORDA DE AÇO</div>
    <div style="overflow-x:auto">
    <table class="pm-table">
        <thead>
            <tr>
...
<meta charset="utf-8">
<title>PCP Molas — Borda de Aço</title>
<style>
