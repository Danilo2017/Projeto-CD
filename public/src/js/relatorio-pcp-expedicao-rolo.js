(function () {
    ...
    const HISTORICO = [
        { data: '31/07/2026', alteracao: 'Elaboração Inicial' },
    ];
...
<div class="pm-header-wrap">
    <div class="pm-header-row1">
        ...
        <div class="col-title">RELATÓRIO DE EXPEDIÇÃO — QUANTIDADE DE ROLO — PCP MOLAS</div>
...
<div class="pm-historico">
    ...
    function renderRelatorio(rows, numLote, dataLote, hoje) {
...
<tr>
    ...
    <td style="text-align:center;font-weight:bold">${fmtD(r.QTE_ROLO)}</td>
</tr>`;
        }

        // ── SEÇÃO 2: AGRUPADO POR ITEM ──────────────────────────────
        const mapaGrupos = new Map();
        for (const r of rows) {
            const chave = `${r.COD_ITEM}||${r.DESC_TECNICA}||${r.MASCARA}`;
            if (!mapaGrupos.has(chave)) {
                mapaGrupos.set(chave, {
                    COD_ITEM:     r.COD_ITEM,
                    DESC_TECNICA: r.DESC_TECNICA,
                    MASCARA:      r.MASCARA,
                    QTDE_OF:      0,
                    QTE_ROLO:     0,
                });
            ...
            g.QTDE_OF  += parseFloat(r.QTDE_OF)  || 0;
            g.QTE_ROLO += parseFloat(r.QTE_ROLO) || 0;
        }
        ...
        for (const g of mapaGrupos.values()) {
            totalOf   += g.QTDE_OF;
            totalRolo += g.QTE_ROLO;
            corpoAgrup += `
<tr>
    <td>${fmt(g.COD_ITEM)}</td>
    <td class="td-wrap">${fmt(g.DESC_TECNICA)}</td>
    <td class="td-wrap">${fmt(g.MASCARA)}</td>
    <td style="text-align:center">${fmtN(g.QTDE_OF)}</td>
    <td style="text-align:center;font-weight:bold">${fmtD(g.QTE_ROLO)}</td>
</tr>`;
        }
        corpoAgrup += `
<tr class="total-row">
    <td colspan="3" style="text-align:right;padding-right:8px">TOTAL GERAL:</td>
    <td style="text-align:center">${fmtN(totalOf)}</td>
    <td style="text-align:center">${fmtD(totalRolo)}</td>
</tr>`;
...
<div class="pm-section">
    ...
    <div class="pm-section-title" style="margin-top:14px">${titulo} — AGRUPADO POR ITEM</div>
    <div style="overflow-x:auto">
    <table class="pm-table">
        <thead>
            <tr>
...
<meta charset="utf-8">
<title>PCP Molas — Expedição Qtde de Rolo</title>
<style>
  ...
  .pm-section:last-child{page-break-after:avoid}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-pcp-expedicao-rolo', {
                    method:  'POST',
                ...
                const hoje = dataHoje();
                const html = renderRelatorio(data.rows || [], numLote, data.data_lote || '', hoje);
