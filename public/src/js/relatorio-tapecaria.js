(function () {
    ...
    /* Sequência de larguras para exibição dos subtítulos de grupo */
    const LARGURA_LABEL = {
        790:  'LARGURA 790',
        1380: 'LARGURA 1380',
        960:  'LARGURA 960',
...
<tr class="total-row">
    ...
    function renderTapecaria(colchaoboxRows, cabeceiraRows, numLote, dataLote, hoje) {
        const temBox  = colchaoboxRows && colchaoboxRows.length > 0;
        const temCab  = cabeceiraRows  && cabeceiraRows.length  > 0;
...
<div class="tap-section">
    ...
    <div class="tap-section-title">
        COLCHÃO BOX - LOTE ${numLote}${dataLote ? ' (' + dataLote + ')' : ''}
    </div>
    <div style="overflow-x:auto">
    <table class="tap-table">
        ${THEAD}
        <tbody>${buildSecao(colchaoboxRows)}</tbody>
    </table>
    </div>
    ${historico()}
</div>`;
...
<div class="tap-section">
    ${cabecalho(hoje)}
    <div class="tap-section-title">
        CABECEIRAS - LOTE ${numLote}${dataLote ? ' (' + dataLote + ')' : ''}
    </div>
    <div style="overflow-x:auto">
    <table class="tap-table">
        ${THEAD}
        <tbody>${buildSecao(cabeceiraRows)}</tbody>
    </table>
    </div>
    ${historico()}
</div>`;
...
<meta charset="utf-8">
<title>Colchão Box — Sequência de Produção</title>
<style>
  ...
  .tap-section:last-child{page-break-after:avoid}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-tapecaria', {
                    method:  'POST',
                ...
                const html = renderTapecaria(data.colchaobox_rows || [], data.cabeceira_rows || [], numLote, data.data_lote || '', hoje);
