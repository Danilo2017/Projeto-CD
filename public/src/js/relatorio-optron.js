(function () {
    ...
    const DOC_OPTRON   = 'R.14.GEP-22';  // Optron
    const DOC_AGRUP    = 'R.14.GEP-23';  // Optron Agrupado
    ...
    /* ── Renderiza relatório Optron ──────────────── */
    function renderOptron(rows, numLote, dataLote) {
        ...
        for (const r of rows) {
            ...
            <tr>
                ...
                <td class="td-wrap">${fmt(r.DESC_TECNICA_FAIXA)}</td>
...
<div class="pcp-section">
    ...
    <div style="display:flex;align-items:center;border:1px solid #000;border-top:none;font-size:8pt">
        ...
        <div style="flex:1;text-align:center;padding:2px 8px;border-right:1px solid #000;font-weight:bold">${DOC_OPTRON}</div>
    ...
    <div class="pcp-section-title">
        OPTRON - LOTE ${numLote}${dataLote ? ' (' + dataLote + ')' : ''}
    </div>
    ...
    /* ── Renderiza Optron Agrupado ───────────────── */
    function renderOptronAgrupado(rows, numLote, dataLote) {
        ...
        corpoTabela += `
        <tr class="total-row">
            <td colspan="5" style="text-align:right">TOTAL LINEAR:</td>
            <td style="text-align:center">${fmtDecimal(totalLinear)}</td>
        </tr>`;
...
<div class="pcp-section">
    ...
    <div class="pcp-section-title">
        OPTRON AGRUPADO - LOTE ${numLote}${dataLote ? ' (' + dataLote + ')' : ''}
    </div>
...
<meta charset="utf-8">
<title>Sequência de Produção — Optron</title>
<style>
  ...
  .pcp-table td.td-wrap{white-space:normal;word-break:break-word}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-optron', {
                    method:  'POST',
                ...
                let html  = renderOptron(data.optron_rows || [], numLote, dt);
                html     += renderOptronAgrupado(data.agrupado_rows || [], numLote, dt);
