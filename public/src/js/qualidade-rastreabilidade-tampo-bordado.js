(function () {
    ...
    /* Aceita apenas P1, P2 e P3 (ou vazio = P1) */
    function isP1P2P3(row) {
        const t = (row.TANQUE || '').trim().toUpperCase();
        return t === '' || /^P[123]$/.test(t);
    }

    /* ── Cabeçalho padrão Rastreabilidade Bordadeira ── */
    function cabecalho(hoje) {
        return `
    <div class="pcp-report-header">
        <div class="col-logo"><img src="${LOGO_URL}" alt="Gazin"></div>
        <div class="col-title">RASTREABILIDADE BORDADEIRA - TAMPO</div>
        <div class="col-right">
            <div><strong>R.15.BOR-01</strong></div>
            <div>REVISÃO:01 DATA:${hoje}</div>
        </div>
    </div>`
    }

    /* ── Tampo Bordado (tabela principal) ─────────── */
    function renderTampoBordado(rows, numLote, dataLote) {
        const filtrado = (rows || []).filter(isP1P2P3);
        ...
        for (const r of filtrado) {
            ...
            <tr>
                ...
                <td class="text-center">${fmt(r.LOTE_ESPUMA)}</td>
                <td class="text-center">${fmt(r.LOTE_TECIDO)}</td>
                <td class="text-center">${fmt(r.LOTE_TNT)}</td>
                <td class="text-center">${fmt(r.LOTE_FIBRA)}</td>
            </tr>`;
        }
        ...
        const loteTitulo = dataLote
            ? `TAMPO BORDADO- LOTE ${numLote} ${dataLote}`
            : `TAMPO BORDADO- LOTE ${numLote}`;
...
<meta charset="utf-8">
<title>Rastreabilidade Bordadeira — Tampo</title>
<style>
...
@media print{body{background:#fff;padding:0}.pcp-section{page-break-after:always;box-shadow:none;border-radius:0;margin:0;padding:10px 14px}.pcp-section:last-child{page-break-after:avoid}.pcp-table td.td-wrap{white-space:normal;word-break:break-word}}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('qualidade-api-rastreabilidade-tampo-bordado', {
                    method:  'POST',
                ...
                const html = renderTampoBordado(data.tampo_rows || [], numLote, dt);

                if (!html) {
                    statusMsg.innerHTML = '<span class="text-warning">Nenhum dado P1/P2/P3 encontrado para este lote.</span>';
                    return;
                }
