(function () {
    ...
    const DOC_LISO     = 'R.14.GEP-24';  // Tampo Liso
    const DOC_AGRUP    = 'R.14.GEP-25';  // Tampo Liso Agrupado
    ...
    /* ── Renderiza relatório Tampo Liso ──────────── */
    function renderTampoLiso(rows, numLote, dataLote) {
        ...
        for (const r of rows) {
            totalQtdeOf    += parseFloat(r.QTDE_OF    || 0);
            ...
            <tr>
                ...
                <td class="td-wrap">${fmt(r.MEDIDA)}</td>
        ...
        <tr class="total-row">
            ...
            <td style="text-align:center">${totalQtdeOf}</td>
            <td colspan="2"></td>
...
<div class="pcp-section">
    ...
    <div style="display:flex;align-items:center;border:1px solid #000;border-top:none;font-size:8pt">
        ...
        <div style="flex:1;text-align:center;padding:2px 8px;border-right:1px solid #000;font-weight:bold">${DOC_LISO}</div>
    ...
    <div class="pcp-section-title">
        TAMPO LISO - LOTE ${numLote}${dataLote ? ' (' + dataLote + ')' : ''}
    </div>
    ...
    /* ── Renderiza Tampo Liso Agrupado ──────────── */
    function renderTampoLisoAgrupado(rows, numLote, dataLote) {
...
<div class="pcp-section">
    ...
    <div style="display:flex;align-items:center;border:1px solid #000;border-top:none;font-size:8pt">
        <div style="width:110px;flex-shrink:0;border-right:1px solid #000;padding:2px 8px"></div>
        <div style="flex:1;text-align:center;padding:2px 8px;border-right:1px solid #000;font-weight:bold">${DOC_AGRUP}</div>
        <div style="width:170px;flex-shrink:0;text-align:right;padding:2px 8px">${REVISAO}&nbsp;&nbsp;DATA: ${DATA_REVISAO}</div>
    </div>
    <div class="pcp-section-title">
        TAMPO LISO AGRUPADO - LOTE ${numLote}${dataLote ? ' (' + dataLote + ')' : ''}
    </div>
...
<meta charset="utf-8">
<title>Sequência de Produção — Tampo Liso</title>
<style>
  ...
  .pcp-table td.td-wrap{white-space:normal;word-break:break-word}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-tampo-liso', {
                    method:  'POST',
                ...
                let html = renderTampoLiso(data.tampo_rows || [], numLote, dt);
                html    += renderTampoLisoAgrupado(data.agrupado_rows || [], numLote, dt);
