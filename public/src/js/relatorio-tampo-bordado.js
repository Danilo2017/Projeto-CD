(function () {
    ...
    /* ── Renderiza relatório Tampo Bordado ───────── */
...
<div class="pcp-section">
    ...
    /* ── Renderiza Tampo Bordado Conjugado ─────── */
    function renderTampoBordadoConj(rows, numLote, dataLote) {
        ...
        for (const r of rows) {
            ...
            <tr>
                <td>${fmt(r.ORD)}</td>
                <td class="text-center">${fmt(r.NUM_ORDEM)}</td>
                <td>${fmt(r.COD_ITEM_PAI)}</td>
                <td class="td-wrap">${fmt(r.DESC_PAI)}</td>
                <td class="td-wrap">${fmt(r.MASCARA_PAI)}</td>
                <td>${fmt(r.COD_ITEM)}</td>
                <td class="text-center">${fmt(r.ID)}</td>
                <td class="td-wrap">${fmt(r.DESC_TECNICA)}</td>
                <td class="td-wrap">${fmt(r.MASCARA)}</td>
...
<div class="pcp-section">
    ...
    /* ── Helpers NLS Oracle (número com vírgula) ── */
    function toNum(v) {
    ...
    function tabelaBordadeira(rows, titulo, numLote, dataLote) {
        ...
        let totalQtde = 0, totalLinear = 0, corpo = '';

        for (const r of rows) {
            totalQtde   += parseFloat(r.QTDE_TAMPO || 0);
            totalLinear += toNum(r.LINEAR) || 0;
            ...
            <tr>
                ...
                <td>${fmt(r.TECIDO)}</td>
                <td>${fmt(r.BORDADO)}</td>
                <td class="text-right">${fmtDec2(r.LINEAR)}</td>
                <td class="text-center">${fmtNum(r.ESPESSURA)}</td>
            </tr>`;
        }
    ...
    <table class="pcp-table">
        <thead>
            <tr>
                <th>COD ITEM</th><th>DESCRIÇÃO TÉCNICA</th><th>MASCARA</th>
                <th>QTI</th><th>TANQUE</th><th>TECIDO</th>
                <th>BORDADO</th><th>METRO LINEAR</th><th>ESPESSURA</th>
            </tr>
        </thead>
        <tbody>${corpo}</tbody>
    </table>
...
<div class="pcp-section">
    ...
    function renderBordadeiraSec(rows, numLote, dataLote) {
        if (!rows || rows.length === 0) return '';
...
<meta charset="utf-8">
<title>Sequência de Produção — Tampo Bordado</title>
<style>
  ...
  .pcp-table td.td-wrap{white-space:normal;word-break:break-word}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-tampo-bordado', {
                    method:  'POST',
