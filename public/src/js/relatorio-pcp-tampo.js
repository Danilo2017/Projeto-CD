...
<div class="pm-header-wrap">
    <div class="pm-header-row1">
        ...
        <div class="col-title">RELATÓRIO DE PRODUÇÃO — QUANTIDADE DE TAMPO — PCP MOLAS</div>
...
<tr>
    ...
    <td>${fmt(r.COD_ITEM_TAMPO)}</td>
    <td class="td-wrap">${fmt(r.DESC_TAMPO)}</td>
    <td class="td-wrap">${fmt(r.MASCARA_TAMPO)}</td>
    <td style="text-align:center;font-weight:bold">${fmtN(r.TOTAL_TAMPO)}</td>
</tr>`;
        }

        // ── SEÇÃO 2: AGRUPADO POR TAMPO ─────────────────────────────
        const mapaGrupos = new Map();
        for (const r of rows) {
            const chave = `${r.COD_ITEM_TAMPO}||${r.DESC_TAMPO}||${r.MASCARA_TAMPO}`;
            if (!mapaGrupos.has(chave)) {
                mapaGrupos.set(chave, {
                    COD_ITEM_TAMPO: r.COD_ITEM_TAMPO,
                    DESC_TAMPO:     r.DESC_TAMPO,
                    MASCARA_TAMPO:  r.MASCARA_TAMPO,
                    TOTAL:          0,
                });
            }
            mapaGrupos.get(chave).TOTAL += parseFloat(r.TOTAL_TAMPO) || 0;
        }
        ...
        for (const g of mapaGrupos.values()) {
            ...
            corpoAgrup += `
<tr>
    <td>${fmt(g.COD_ITEM_TAMPO)}</td>
    <td class="td-wrap">${fmt(g.DESC_TAMPO)}</td>
    <td class="td-wrap">${fmt(g.MASCARA_TAMPO)}</td>
    <td style="text-align:center;font-weight:bold">${fmtN(g.TOTAL)}</td>
</tr>`;
        }
        corpoAgrup += `
<tr class="total-row">
    <td colspan="3" style="text-align:right;padding-right:8px">TOTAL GERAL DE TAMPOS:</td>
...
<div class="pm-section">
    ...
    <div class="pm-section-title" style="margin-top:14px">${titulo} — AGRUPADO POR TAMPO</div>
    <div style="overflow-x:auto">
    <table class="pm-table">
        <thead>
            <tr>
...
<meta charset="utf-8">
<title>PCP Molas — Quantidade de Tampo</title>
<style>
  ...
  .pm-section:last-child{page-break-after:avoid}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-pcp-tampo', {
                    method:  'POST',
