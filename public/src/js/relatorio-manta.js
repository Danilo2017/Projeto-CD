(function () {
    ...
    const DOC_CODE     = 'R.14.GEP-31';  // Manta
    ...
    /* ── Renderiza uma seção de manta (título variável) ── */
    function renderSecaoManta(rows, titulo, numLote, dataLote) {
        if (!rows || rows.length === 0) return '';

        let totalQtde      = 0;
        let totalQtdeManta = 0;
        let corpoTabela    = '';

        for (const r of rows) {
            totalQtde      += parseFloat(r.QTDE       || 0);
            totalQtdeManta += parseFloat(r.QTDE_MANTA || 0);
            ...
            <tr>
                ...
                <td class="text-center">${fmtNum(r.QTDE_MANTA)}</td>
            </tr>`;
        }
        ...
        <tr class="total-row">
            ...
            <td colspan="4"></td>
            <td style="text-align:center">${totalQtdeManta}</td>
        </tr>`;
...
<meta charset="utf-8">
<title>Sequência de Produção — Manta</title>
<style>
  ...
  .pcp-table td.td-wrap{white-space:normal;word-break:break-word}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-manta', {
                    method:  'POST',
                ...
                let html  = renderSecaoManta(data.manta_rows      || [], 'MANTA',      numLote, dt);
                html     += renderSecaoManta(data.manta_mesa_rows || [], 'MANTA MESA', numLote, dt);
