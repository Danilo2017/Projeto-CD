(function () {
    ...
    /* ── Renderiza rastreabilidade Cordão de Molas ── */
    function renderCordao(rows, numLote) {
        const hoje = dataHoje();
        ...
        if (rows && rows.length > 0) {
            ...
            <tr>
                ...
                <td class="text-center">${fmt(r.COD_ITEM_CORDAO)}</td>
                <td class="td-wrap">${fmt(r.DESC_CORDAO)}</td>
                <td class="text-center">${fmt(r.MASCARA_CORDAO)}</td>
                <td class="text-center">${fmtN(r.TOTAL_MOLINHA)}</td>
                <td></td>
                <td></td>
            </tr>`;
            }
        } else {
            corpoTabela = '<tr><td colspan="8" class="text-center text-muted">Nenhum dado encontrado para este lote.</td></tr>';
        }
...
<div class="rastr-section">
    <div class="rastr-header">
        ...
        <div class="col-title">RASTREABILIDADE - FABRICAÇÃO CORDÃO DE MOLAS</div>
        <div class="col-setor">
    ...
    <div class="rastr-subheader">
        <div class="sub-row1">
            <span class="sub-code">R.16.MOL-02</span>
    ...
    <table class="rastr-table">
        <thead>
            <tr>
                <th>DATA</th>
                <th>Nº DO PEDIDO /<br>LOTE DE PRODUÇÃO</th>
                <th>CÓD. CORDÃO</th>
                <th>DESC. CORDÃO</th>
                <th>MÁSCARA<br>CORDÃO</th>
                <th>TOTAL<br>CORDÕES</th>
                <th>MÁQUINA</th>
                <th>NF / LOTE COLA</th>
            </tr>
        </thead>
...
<meta charset="utf-8">
<title>Rastreabilidade — Cordão de Molas</title>
<style>
...
.rastr-table td{border:1px solid #ccc;padding:3px 6px;white-space:nowrap;min-height:18px;height:18px}
  ...
  .rastr-section{box-shadow:none;border-radius:0;margin:0;padding:10px 14px}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('qualidade-api-rastreabilidade-cordao-molas', {
                    method:  'POST',
                ...
                printArea.innerHTML = renderCordao(data.rows || [], numLote);
