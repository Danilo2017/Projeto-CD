(function () {
    ...
    const HISTORICO = [
        { data: '13/08/2026', alteracao: 'Elaboração Inicial' },
    ];
    ...
    function renderCaixoteMola(rows, numLote, dataLote) {
        ...
        const titulo = dataLote
            ? `CAIXOTE DE MOLA - LOTE ${numLote} (${dataLote})`
            : `CAIXOTE DE MOLA - LOTE ${numLote}`;

        return `
<div class="pcp-section">
    <div class="pcp-report-header">
        <div class="col-logo"><img src="${LOGO_URL}" alt="Gazin"></div>
        <div class="col-title">RASTREABILIDADE - CAIXOTE DE MOLA</div>
        <div class="col-right">
            <div><strong>R.15.MOL-01</strong></div>
    ...
    <table class="pcp-table">
        <thead>
            <tr>
                ...
                <th>ALT MOLA</th>
                <th>ALT EPS</th>
                <th>LOTE DE ESPUMA<br>(BORDA 2)</th>
                <th>LOTE/NF DO MOLEJO<br>(ENSACADA OU BONNEL)</th>
                <th>LOTE DA COLA</th>
                <th>LOTE DO<br>FELTRO/TNT</th>
                <th>LOTE EPS</th>
            </tr>
        </thead>
...
<meta charset="utf-8">
<title>Rastreabilidade — Caixote de Mola</title>
<style>
...
.pcp-table td.td-blank{min-width:70px;height:16px}
...
@media print{body{background:#fff;padding:0}.pcp-section{page-break-after:always;box-shadow:none;border-radius:0;margin:0;padding:10px 14px}.pcp-section:last-child{page-break-after:avoid}.pcp-table td.td-wrap{white-space:normal;word-break:break-word}}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('qualidade-api-rastreabilidade-caixote-mola', {
                    method:  'POST',
                ...
                const html = renderCaixoteMola(data.rows || [], numLote, data.data_lote || '');
