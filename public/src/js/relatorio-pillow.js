(function () {
    ...
    /* ── Renderiza seção PILLOW ──────────────────── */
    function renderSecaoPillow(rows, numLote, dataLote) {
        ...
        for (const r of rows) {
            ...
            <tr>
                ...
                <td class="text-center">${fmt(r.TIPO_PILLOW)}</td>
                <td class="text-center">${fmt(r.COD_ITEM_TECIDO)}</td>
                <td class="text-center">${fmtNum(r.ALT_PILLOW)}</td>
...
<meta charset="utf-8">
<title>Sequência de Produção — Pillow</title>
<style>
  ...
  .pcp-table td.td-wrap{white-space:normal;word-break:break-word}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-pillow', {
                    method:  'POST',
                ...
                printArea.innerHTML = renderSecaoPillow(data.pillow_rows || [], numLote, data.data_lote || '');
