    ...
    function initDataTables() {
        if (typeof $ !== 'undefined' && $.fn.DataTable) {
            $('.table').each(function() {
                ...
                // Ignorar tabelas que serão inicializadas manualmente pelos módulos de relatório/cadastro
                // 1. Tabelas com IDs específicos gerenciados por JS próprio
                // 2. Tabelas dentro do container de comissão
                // 3. Tabelas cujo tbody contenha colspan (placeholder de dados dinâmicos)
                if (tableId === 'tabelaProdutividade' || 
                    tableId === 'tabelaApontamentos' ||
                    tableId === 'tabelaVinculos' ||
                    tableId === 'tabelaCentros' ||
                    tableId.indexOf('tabela') === 0 ||
                    table.closest('.comissao-dashboard-container').length > 0 ||
                    table.find('tbody td[colspan]').length > 0) {
                    return;
                }
