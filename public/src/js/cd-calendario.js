...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Máscara monetária para Valor Pago
    document.getElementById('valorPago').addEventListener('input', function() {
        ...
        this.value = 'R$ ' + valor.replace('.', ',').replace(/\B(?=(\d{3})+(?!\d))/g, '.');
    });
});
...
async function salvarRecibo(event) {
    ...
    // Extrair valor numérico do campo com máscara monetária
    const valorPagoRaw = document.getElementById('valorPago').value;
    const valorPagoNumerico = parseFloat(valorPagoRaw.replace(/[R$\s.]/g, '').replace(',', '.'));
    ...
    if (!dados.empresa_pagadora || isNaN(dados.valor_pago) || dados.valor_pago <= 0) {
...
async function visualizarRecibo(id) {
    try {
        ...
        if (result.success) {
            ...
            document.getElementById('modalVisualizarRecibo').style.display = 'block';
            document.body.style.overflow = 'hidden';
        } else {
 ...
 * Se vier apenas números, aplica a máscara.
 */
function formatarCnpjCpf(valor) {
    ...
    if (apenasNumeros.length <= 11) {
        ...
        // CNPJ: 00.000.000/0000-00
        return apenasNumeros.substring(0, 14)
            .replace(/^(\d{2})(\d)/, '$1.$2')
                ...
                <div class="recibo-modelo">
                    ...
                    <div class="recibo-corpo" style="margin-bottom: 16px;">
                        <p class="recibo-texto" style="font-size: 1rem; margin-bottom: 8px;">
                            <strong>Recebí(emos)</strong> de <strong>${recibo.EMPRESA_PAGADORA}</strong>${recibo.CNPJ_CPF ? ` - CPF/CNPJ nº ${formatarCnpjCpf(recibo.CNPJ_CPF)}` : ''}, a importância de <strong>${valorExtenso}</strong> referente a(o) ${descricaoDescarga}. Pagamento: ${formaPagamentoTexto}
                        </p>
