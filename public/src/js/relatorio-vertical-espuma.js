(function () {
    ...
    const HISTORICO = [
        { data: '27/11/2025', alteracao: 'Elaboração Inicial' },
    ];

    const LOGO_URL     = 'https://system.colchoesgazin.com.br/assets/media/logos/logo-gazin.png';
    const DOC_CODE     = 'R.14.GEP-33';  // Vertical Espuma
    ...
    function renderVerticalEspuma(rows, numLote, dataLote) {
        ...
        for (const r of rows) {
            ...
            <tr>
                <td>${fmt(r.ORD)}</td>
                <td class="text-center">${fmt(r.LOTE)}</td>
                <td class="text-center">${fmt(r.NUM_ORDEM)}</td>
                <td class="td-wrap">${fmt(r.DESCRICAO)}</td>
                <td class="td-wrap">${fmt(r.DESC_TECNICA)}</td>
                <td class="td-wrap">${fmt(r.MASCARA_ITEM)}</td>
        ...
        corpo += `
        <tr class="total-row">
            <td colspan="6" style="text-align:right">QTDE TOTAL:</td>
            <td style="text-align:center">${totalQtde}</td>
        </tr>`;

        const titulo = `VERTICAL – ESPUMA - LOTE ${numLote}${dataLote ? ' (' + dataLote + ')' : ''}`;
...
<meta charset="utf-8">
<title>Sequência de Produção — Vertical Espuma</title>
<style>
...
.pcp-table{width:100%;border-collapse:collapse;font-size:8pt;table-layout:fixed}
.pcp-table th{background:#1f3864;color:#fff;border:1px solid #999;padding:2px 4px;text-align:center;font-weight:bold;-webkit-print-color-adjust:exact;print-color-adjust:exact}
.pcp-table td{border:1px solid #ccc;padding:2px 4px;white-space:normal;word-break:break-word}
.pcp-table th:nth-child(1),.pcp-table td:nth-child(1){width:8%}
.pcp-table th:nth-child(2),.pcp-table td:nth-child(2){width:6%}
.pcp-table th:nth-child(3),.pcp-table td:nth-child(3){width:10%}
.pcp-table th:nth-child(4),.pcp-table td:nth-child(4){width:30%}
.pcp-table th:nth-child(5),.pcp-table td:nth-child(5){width:20%}
.pcp-table th:nth-child(6),.pcp-table td:nth-child(6){width:20%}
.pcp-table th:nth-child(7),.pcp-table td:nth-child(7){width:6%}
  ...
  .pcp-section:last-child{page-break-after:avoid}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnGerar.addEventListener('click', async function () {
            ...
            try {
                const res  = await fetch('pcp-api-relatorio-vertical-espuma', {
                    method:  'POST',
                ...
                const html = renderVerticalEspuma(data.rows || [], numLote, data.data_lote || '');
