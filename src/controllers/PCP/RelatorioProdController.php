...
class RelatorioProdController extends Controller
{
    ...
    public function indexDiscoCorte(): void
    {
        $this->render('pcp/relatorio-disco-de-corte', []);
    }
    ...
    public function buscarDiscoCorte(): void
    {
        try {
            $body    = self::getBody() ?? [];
            $numLote = (int) ($body['num_lote'] ?? 0);
            ...
            $result = RelatorioProdHandler::buscarDiscoCorte($emprId, $numLote);
            self::response($result, 200);
        } catch (\Exception $e) {
            $code = is_numeric($e->getCode()) ? (int) $e->getCode() : 0;
            self::response(['error' => $e->getMessage()], $code ?: 500);
    ...
    public function indexPcpTampo(): void
    {
        $this->render('pcp/relatorio-pcp-tampo', []);
    }

    public function buscarPcpTampo(): void
    {
        try {
            $body    = self::getBody() ?? [];
            $numLote = (int) ($body['num_lote'] ?? 0);
            $emprId  = $this->emprIdSessao();
            if ($numLote <= 0) throw new \Exception('Número do lote é obrigatório.', 400);
            $result = RelatorioProdHandler::buscarPcpTampo($emprId, $numLote);
            self::response($result, 200);
        } catch (\Exception $e) {
            $code = is_numeric($e->getCode()) ? (int) $e->getCode() : 0;
            self::response(['error' => $e->getMessage()], $code ?: 500);
    ...
    public function buscarPcpBordaAco(): void
    {
        try {
            $body    = self::getBody() ?? [];
            $numLote = (int) ($body['num_lote'] ?? 0);
            $emprId  = $this->emprIdSessao();
            if ($numLote <= 0) throw new \Exception('Número do lote é obrigatório.', 400);
            $result = RelatorioProdHandler::buscarPcpBordaAco($emprId, $numLote);
            self::response($result, 200);
        } catch (\Exception $e) {
            $code = is_numeric($e->getCode()) ? (int) $e->getCode() : 0;
            self::response(['error' => $e->getMessage()], $code ?: 500);
